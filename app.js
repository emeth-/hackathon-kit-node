    ...
    request(options, function (error, response, body) {
        ...
        if (hooks.length && update) {
            _.each(hooks, function (hook) {
                    var putOptions = {
                        method: 'PUT',
                        url: 'https://api.trello.com/1/webhooks/' + hook.id + '?active=true&description=' + hook.description + '&idModel=' + hook.idModel + '&callbackURL=' + trelloUrlBase + 'trello-move&key=' + trelloKey + '&token=' + trelloToken,
                        headers: {
                            'user-agent': 'node.js',
                            'Content-Type': 'application/json;charset=UTF-8'
                        },
